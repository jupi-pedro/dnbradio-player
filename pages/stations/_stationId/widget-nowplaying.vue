    ...
    switch (station.value.nowplaying_url.type) {
      case 'azuracast':
        nowplaying.value = {
          ...
          streamer: res.now_playing.streamer_name,
          label: null
        };
      break;
      case 'sam':
      ...
      break;
      case 'icecast':
        let titleParts = res.icestats.source.title.split(' - ')
        nowplaying.value.artist = titleParts[0]
        nowplaying.value.title = titleParts[1]
        nowplaying.value.listeners = res.icestats.source.listeners
        nowplaying.value.albumyear = null
        nowplaying.value.album = null
        nowplaying.value.song_type = 'S'
...
const filteredData = computed(() => {
  ...
  return data.value
  .filter((item: any) => {
    var end = moment.utc({ hour:23, minute:59, second:59 } as any).add(1, 'days');
